...
try: 
    ...
    st.error("Missing dependency: scikit-learn. Run 'pip install scikit-learn' to fix."); st.stop()
...
# Regression for Confidence Metric
...
st.title(f"{selected_region} | Technical Intelligence")
st.markdown("---")
...
if "Precipitation" in analysis_mode or "Full" in analysis_mode:
    fig_main.add_trace(go.Bar(x=df['Year'], y=df['Rain_Anomaly_mm'], name="Precipitation", marker_color='#00d2ff', opacity=0.4), secondary_y=False)

if "Temperature" in analysis_mode or "Full" in analysis_mode:
    fig_main.add_trace(go.Scatter(x=df['Year'], y=df['Temp_Anomaly_C'], name="Temperature", line=dict(color='#ff4b4b', width=3)), secondary_y=True)
    ...
    fig_main.add_trace(go.Scatter(x=np.concatenate([fut_x.flatten(), fut_x.flatten()[::-1]]),
                                  ...
                                  fill='toself', fillcolor='rgba(255, 75, 75, 0.1)', line_color='rgba(0,0,0,0)', showlegend=False), secondary_y=True)
...
fig_main.add_trace(go.Scatter(x=outliers['Year'], y=outliers['Temp_Anomaly_C'] if "Temp" in analysis_mode else outliers['Rain_Anomaly_mm'],
                               mode='markers', name='Extreme Event', marker=dict(color='#ffa500', size=12, symbol='diamond')), 
...
with c_map:
    ...
    # FIX: Rename columns to lowercase for Streamlit API compatibility
    map_df = df[['Lat', 'Lon']].head(1).rename(columns={'Lat': 'lat', 'Lon': 'lon'})
    st.map(map_df, zoom=6)
    st.caption(f"Coordinates: {df['Lat'].iloc[0]}°N, {df['Lon'].iloc[0]}°W")

with c_cycle:
    st.markdown(f'<p class="sector-header">Seasonal Profile ({selected_year})</p>', unsafe_allow_html=True)
    ...
    fig_cycle = go.Figure()
    fig_cycle.add_trace(go.Scatter(x=months, y=clim_r, name="LTM Mean", line=dict(color='gray', dash='dash')))
    fig_cycle.add_trace(go.Scatter(x=months, y=curr_r, name=f"Observed {selected_year}", line=dict(color='#00d2ff', width=3)))
    fig_cycle.update_layout(template="plotly_dark", paper_bgcolor='rgba(0,0,0,0)', height=300, margin=dict(t=0, b=0), legend=dict(orientation="h", y=-0.2))
...
# --- 7. EXPORT ---
st.sidebar.divider()
st.sidebar.download_button("📂 Download Pro Report", df.to_csv(index=False), f"GCI_Report_{selected_region}.csv")